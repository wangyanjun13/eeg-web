...
class PreprocessService:
    ...
    def apply_filter(self, dataset_id: str, subject_id: str, params: FilterParams) -> PreprocessedData:
        """应用滤波器"""
    ...
    def run_ica(self, dataset_id: str, subject_id: str, params: ICAParams) -> PreprocessedData:
        """运行ICA分析"""
        raw = self.dataset_service._read_eeg_file(dataset_id, subject_id)
        # ICA实现
        return self._convert_to_preprocessed_data(raw, ["ica"])

    def remove_artifacts(self, dataset_id: str, subject_id: str, params: ArtifactParams) -> PreprocessedData:
        """去除伪迹"""
        raw = self.dataset_service._read_eeg_file(dataset_id, subject_id)
        # 伪迹去除实现
